...
is no "BM25 only" mode of the service. The fourth mirrors the live pipeline
...
def _vector_candidates(query: str, owner_id: UUID, limit: int) -> tuple[list[str], float]:
    start = perf_counter()
    hits = vector_search.search(query, limit, owner_id)
    return [chunk_id for chunk_id, _ in hits], (perf_counter() - start) * 1000


def _bm25_candidates(query: str, owner_id: UUID, limit: int) -> tuple[list[str], float]:
    start = perf_counter()
    hits = bm25_index.search(query, limit, owner_id)
    return [chunk_id for chunk_id, _ in hits], (perf_counter() - start) * 1000


def _fuse(vector_ids: list[str], bm25_ids: list[str]) -> tuple[list[str], float]:
    start = perf_counter()
    fused = reciprocal_rank_fusion(
        ranked_lists={VECTOR_SOURCE: vector_ids, BM25_SOURCE: bm25_ids},
        weights={
...
def run_vector_only(
    ...
    del session
    chunk_ids, elapsed = _vector_candidates(query, owner_id, top_k)
...
def run_bm25_only(
    ...
    del session
    chunk_ids, elapsed = _bm25_candidates(query, owner_id, top_k)
    return RetrievalOutput(chunk_ids[:top_k], StageTimings(bm25_ms=elapsed))
...
def run_hybrid(
    ...
    vector_ids, vector_ms = _vector_candidates(
        query, owner_id, settings.VECTOR_CANDIDATE_COUNT
    )
    bm25_ids, bm25_ms = _bm25_candidates(
        query, owner_id, settings.BM25_CANDIDATE_COUNT
    )
    fused_ids, fusion_ms = _fuse(vector_ids, bm25_ids)

    return RetrievalOutput(
        fused_ids[:top_k],
        StageTimings(vector_ms=vector_ms, bm25_ms=bm25_ms, fusion_ms=fusion_ms),
    )
...
def run_hybrid_reranked(
    ...
    reranked = sorted(
        zip(ordered, scores), key=lambda pair: pair[1], reverse=True
    )
...
CONFIGURATIONS: list[tuple[str, str, ConfigurationRunner]] = [
    ("bm25_only", "Lexical baseline (BM25)", run_bm25_only),
    ("vector_only", "Dense baseline (bi-encoder embeddings)", run_vector_only),
    ("hybrid_rrf", "BM25 + dense, Reciprocal Rank Fusion", run_hybrid),
    (
        "hybrid_reranked",
